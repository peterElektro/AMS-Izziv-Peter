...
def train(model, dataset, output_dir, epochs,
    ...
    for epoch in range(epochs):
        ...
        mean_dice, std_dice, _, _ = validate(
            model,
            val_loader,
            device
        )
        ...
        print(
            ...
            f"Val Dice mean={mean_dice:.4f}, std={std_dice:.4f}"
        )
    ...
    print(
        f"[run_train] Training complete. "
        f"Final model saved to: {output_dir / 'model_final.pth'}"
    )
...
def load_dataset(dataset_dir):
    ...
    # nnU-Net: 001_0000.nii.gz -> 001
    def prefixes(paths):
        return [p.name.replace("_0000.nii.gz", "") for p in paths]

    train_ids = prefixes(images_tr)
    val_ids = prefixes(images_val)

    print(
        f"[run_train] Found {len(train_ids)} train ids "
        f"and {len(val_ids)} val ids"
    )
